...
Looking at the singular values, eigenvalues and their interia:
...
## Factor Scores 
- Factor scores can provide insight 
...
$$ \hat{\sigma_{F^*}}^2 = \frac{1}{L} \left( \sigma_\ell^L \left( F_\ell^* - \overline{F}^* \right) \circ \left( F_\ell^* - \overline{F}^* \right)  \right)$$

and lastly the bootstrap ratios (Student's t statistic), $T^* = \overline{F}^* / \hat{\sigma_{F^*}}$
...
- plots can be generated through the console by using the following methods:  `plot_compromise`, `plot_loading`, `plot_partial_fac`, and `plot_ev` 
