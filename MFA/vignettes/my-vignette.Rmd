...
plot_compromise.R generates a plot of the common factor scores. An example is shown below for a toy data set relating to wines. This plot shows how the different objects in the data set (in this case wines, numbered 1 through 12)  vary over the first two dimensions (analogous to principal components) computed from the MFA. 

```{r, fig.cap="Compromise of 10 tables included in a toy data set", fig.show='hold', fig.width=4, echo=FALSE}
plot_compromise(a, textcolor=rainbow(length(a$alpha)))
```

plot_loading.R generates a plot of the variable loadings unique to each individual table. Below is an example of the variable loadings plot for table 1 of the sample data set. Note that these coordinates correspond to the variable loadings data displayed in the sample print method.  

```{r, fig.cap="Variable loadings for Table 1 of 10, plotted on the compromise space generated by the first two dimensions of the MFA", fig.show='hold', fig.width=4, echo=FALSE}
plot_loading(a)
```

Similarly plot_partial_fac.R generates a plot of the partial factor scores unique to each individual table. Below is a plot of the partial factor scores plot for table 1 of the sample data set.   

```{r, fig.cap="Partial factor scores for Table 1 of 10, plotted on the compromise space generated by the first two dimensions of the MFA", fig.show='hold', fig.width=4, echo=FALSE}
plot_partial_fac(a, textcolor=rainbow(length(a$alpha)))
```

plot_ev.R generates a bar graph of the eigenvalues computed in the MFA: 

```{r, fig.cap="Eigenvalues for the toy data set", fig.show='hold', fig.width=4, echo=FALSE}
plot_ev(a)
```

And finally, plot_inertia_pie.R generates a pie chart showing the variance explained by each of the components, similar to the bar graph of the eigenvalues, but visualized in a different way:

```{r, fig.cap="Pie chart of inertia", fig.show='hold', fig.width=4, echo=FALSE}
plot_inertia_pie(a)
```
...
There are several different methods included to explore various descriptive statistics. 

Reduce all these tables by assigning to values first

Lg Table:

```{r, echo=FALSE}
knitr::kable(lg_table(d,s))
```

Rv Table: 

```{r, echo=FALSE}
knitr::kable(rv_table(d,s))
```

Tables by contribution to dimensions:

```{r, echo=FALSE}
knitr::kable(head(table_dim(a)))
```

Variables by contributions to dimensions: 

```{r, echo=FALSE}
knitr::kable(head(var_dim(a)))
```

Observations by contributions to dimensions: 

```{r, echo=FALSE}
knitr::kable(head(obs_dim(a)))
```
...
We included our toy data set with the package for the users to work with. The function loadWineData.R accesses the included dataset.  
